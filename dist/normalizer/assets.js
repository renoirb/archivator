 ...
 */exports.default=(sourceDocument,asset)=>{// console.log(`Given: ${sourceDocument}, Asset: ${asset}`); // DEBUG
let sourceDocumentUrlObj={};try{/**
     ...
     *      ... }
     *
     */sourceDocumentUrlObj=new _url.URL(sourceDocument);// console.log(sourceDocumentUrlObj); // DEBUG
}catch(err){throw new Error(sourceDocument,err)}let targetGiven=String(sourceDocumentUrlObj.href).replace(/([a-z0-9_\-.:])$/i,'$1/');let targetAsset=String(asset).replace(/^\.\//,'');const hasFileExtensionRegEx=/\.(html?|do|action)/;const startWithHttpsRegEx=/^https:\/\//;const startWithHttpMaybeTlsRegEx=/^https?:\/\//;const startWithDoublySlashRegEx=/^\/\//;const startWithOneSlashRegEx=/^\/(?!\/)/;const endWithOneSlashRegEx=/\/(?!\/)$/;/**
   * File with an extension (e.g. /index.html, /action.do)
   * should not be treated as folders.
   */if(hasFileExtensionRegEx.test(sourceDocumentUrlObj.pathname)===true){targetGiven=String(`${sourceDocumentUrlObj.origin}/`);if(startWithOneSlashRegEx.test(asset)===false){// If asset DOES NOT start with a slash (e.g. <img src="a.png" />)
// An array out of the URL, without empty members
// e.g. '/a/c.html' => [ '', 'a', 'c.html' ]
const targetAssetPathnameArray=sourceDocumentUrlObj.pathname.split('/').filter(n=>n);targetAssetPathnameArray.pop();// Strip off file and extension member
targetGiven+=targetAssetPathnameArray.join('/');if(endWithOneSlashRegEx.test(targetGiven)===false){targetGiven+='/'}}// Make sure targetGiven ENDS with slash. http://example.org/a/
}/**
   * If targetAsset starts by /, we explicitly want
   * to start directly from the top of the
   * folder hirerarchy
   * e.g. /a.png
   */if(startWithOneSlashRegEx.test(targetAsset)){// console.log(`Start from top top most parent directory "${targetAsset}"`); // DEBUG
targetGiven=String(`${sourceDocumentUrlObj.protocol}//${sourceDocumentUrlObj.hostname}/`);targetAsset=targetAsset.replace(startWithOneSlashRegEx,'');// Make sure if we had a / at the asset, it's stripped off uniformally
// {asset: '/a.png', targetGiven: 'http://example.org/', targetAsset: 'a.png'}
   ...
   *     sliceUntilHowMany: 0 } ]
   * ```
   */if(/\.\.\//.test(targetAsset)){const goUp=targetAsset.match(/\.\.\//g);// console.log(`Has go up parent operator in path "${targetAsset}", found ${goUp.length} times`); // DEBUG
const tempTargetGivenUrlObj=new _url.URL(targetGiven);const targetGivenPathname=tempTargetGivenUrlObj.pathname.split('/').filter(n=>n);const isTargetAssetGoUpOverflow=targetGivenPathname.length<goUp.length;let sliceUntilHowMany=targetGivenPathname.length-goUp.length;// Make sure this edge-case is taken into account so that we can still
// calculate how deep we go without overriding.
// Maybe that's a mutually exclusive scenario and we need to else from here.
if(isTargetAssetGoUpOverflow===true){// Assuming we have impossible to reach, we got to stop at the origin:
// targetGiven = http://example.org/
// targetAsset = ../../bogus.png
// Force so we serve at the top http://example.org/bogus.png
sliceUntilHowMany=0}const newTargetGivenPathname=targetGivenPathname.slice(0,sliceUntilHowMany);const targetAssetArray=newTargetGivenPathname.concat(targetAsset.split('/').filter(n=>n!=='..'));// console.log([{sourceDocument, targetGiven}, {asset, targetAsset}, {isTargetAssetGoUpOverflow, targetGivenPathname: targetGivenPathname.length, goUp: goUp.length, targetAssetArray, newTargetGivenPathname, sliceUntilHowMany}]);
targetGiven=String(`${sourceDocumentUrlObj.origin}/`);targetAsset=String(`${targetAssetArray.join('/')}`)}/**
   * If targetAsset starts by https?://, we ignore targetGiven
   */if(startWithHttpMaybeTlsRegEx.test(asset)===true){targetGiven=''}/**
   * If targetAsset starts by //, we ignore targetGiven
   */if(startWithDoublySlashRegEx.test(asset)===true){targetGiven=startWithHttpsRegEx.test(sourceDocumentUrlObj.href)===true?'https:':'http:'}const ret=String(`${targetGiven}${targetAsset}`);// console.log('normalizer/assets', ret); // DEBUG
return ret};
