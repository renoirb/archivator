...
const recv=yield(0,_nodeFetch2.default)(src);if(recv.ok===true){console.log(`        dest: ${fileName}`);const dest=fs.createWriteStream(fileName);recv.body.pipe(dest);console.log(`        status: OK`)}else{console.log(`        status: ERR, could not download.`)}}});return function download(_x11){return _ref7.apply(this,arguments)}})();let readCached=(()=>{var _ref8=_asyncToGenerator(function*(file){const data=yield fs.readFile(file,'utf8');return data});return function readCached(_x12){return _ref8.apply(this,arguments)}})();let main=(()=>{var _ref9=_asyncToGenerator(function*(sourceList){for(const source of sourceList){console.log(`  ----`);const cachedFilePath=`archive/${source.slug}`;// Make parent folder configurable #TODO
console.log(`  - source: ${source.url}`);console.log(`    path:   ${cachedFilePath}/`);const cachedFileName=`${cachedFilePath}/cache.html`;const cached=yield readCached(cachedFileName).catch(readCachedError);const matches=yield extractAssets(cached);const assets=yield handleAssets(matches,source);const links=yield extractLinks(cached,source);const cacheJsonRepresentation={source,assets,links};// cacheJsonRepresentation.matches = matches; // DEBUG
const cacheJsonFile=`${cachedFilePath}/cache.json`;if((yield fs.exists(cacheJsonFile))===false){yield fs.writeTextFile(cacheJsonFile,JSON.stringify(cacheJsonRepresentation),'utf8')}yield downloadAssets(assets);// Hacky. For now. I'll fix this soon.
